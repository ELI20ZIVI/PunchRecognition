...
# Trova tutti i file CSV nella cartella Rear_uppercut
csv_files = sorted(glob.glob('Rear_uppercut_split/punch_*.csv'), 
                   key=lambda x: int(os.path.basename(x).replace('punch_', '').replace('.csv', '')))

print(f"Trovati {len(csv_files)} file nella cartella Rear_uppercut")
...
# Configura il layout
fig.update_layout(
    title=f"Tutti i canali IMU - RearHook ({len(csv_files)} misurazioni)",
...
print(f"Visualizzati {len(csv_files)} grafici dalla cartella Rear_hook_split")
