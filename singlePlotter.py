...
# Trova tutti i file CSV nella cartella LeadHook
csv_files = sorted(glob.glob('LeadHook_split/punch_*.csv'))

print(f"Trovati {len(csv_files)} file nella cartella LeadHook")
...
fig.write_html('grafico_interattivo.html', config=config)

print(f"Grafico salvato in 'grafico_interattivo.html'")
print(f"Visualizzati {len(csv_files)} grafici dalla cartella LeadHook")
