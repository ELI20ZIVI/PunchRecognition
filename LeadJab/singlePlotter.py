...
# Trova tutti i file CSV nella cartella Lead_jab
csv_files = sorted(glob.glob('Lead_jab_split/punch_*.csv'), 
                   key=lambda x: int(os.path.basename(x).replace('punch_', '').replace('.csv', '')))

print(f"Trovati {len(csv_files)} file nella cartella Lead_jab")
