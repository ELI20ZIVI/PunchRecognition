...
START = 0
END = 0
ORIGINAL_PATH = "Rear_hook.csv"
FILTERED_PATH = "Rear_hook_filtered.csv"
SPLIT_PATH = "Rear_hook_split"
...
with open(FILTERED_PATH, 'r', newline='', encoding='utf-8') as file:
    ...
    header = next(reader)
    indice_accx = header.index("accx(g)")
    numero_riga = 1
    
    for riga in reader:
        valore_accx = riga[indice_accx]
        
        if valore_accx != '0.0':
            START = numero_riga - 70
            END = START + 180
            ...
            with open(FILTERED_PATH, 'r', newline='', encoding='utf-8') as temp_file:
                ...
                for i in range(min(END, len(righe) - 1), START, -1):
                    if i < len(righe) and righe[i][indice_accx] == '0.0':
                        zeroCounter += 1
                    else:
                        break

            END = END - zeroCounter + 81
