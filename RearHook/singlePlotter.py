...
# Trova tutti i file CSV nella cartella RearHook
csv_files = sorted(glob.glob('Rear_hook_split/punch_*.csv'), 
                   key=lambda x: int(os.path.basename(x).replace('punch_', '').replace('.csv', '')))

print(f"Trovati {len(csv_files)} file nella cartella RearHook")
