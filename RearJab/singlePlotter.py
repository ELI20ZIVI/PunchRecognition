...
# Trova tutti i file CSV nella cartella Rear_jab
csv_files = sorted(glob.glob('Rear_jab_split/punch_*.csv'), 
                   key=lambda x: int(os.path.basename(x).replace('punch_', '').replace('.csv', '')))

print(f"Trovati {len(csv_files)} file nella cartella Rear_jab")
